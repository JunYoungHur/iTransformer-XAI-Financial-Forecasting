...
| 시점 독립 게이트 · 가중치 없음 | 33.8 % | 23.3 % | 1.2 % |
...
- **게이트 종류 비교.** 같은 조건(가중치 없음)에서 GRU 게이트가 시점 독립 게이트보다
  정확도가 높고(38.2% vs 33.8%), 확신 베팅의 long/short 균형도 훨씬 낫다(49.6% vs
  1.2% — 시점 독립 게이트는 확신 베팅이 거의 한 방향으로 쏠림). 단일 실행 기준이라
  경향성 수준의 관찰이며, seed 반복 검증은 남은 과제다.
- **게이트가 보는 것.** 학습된 게이트는 변동성(ATR), 거시 국면(VIX, 장단기 금리차),
  방어 섹터에 무게를 둔다 — 경제적으로 납득되는 묶음.

아래는 종목 A에 대한 시점별 변수 가중치(VSN) 변화다. 왼쪽(GRU)은 특정 변수가
국면에 따라 크게 출렁이는 반면, 오른쪽(시점 독립)은 좁은 띠 안에서 거의 평탄하다.
맥락을 반영하도록 설계한 GRU 게이트가 실제로 시점마다 다른 변수에 무게를 옮긴다는
근거다.
...
실행별 상세 지표(혼동행렬, 확신 구간별 정확도 등)는 `results/`의 JSON에 있다. 단,
VSN 가중치는 게이팅 단계의 변수 강조도이며, 이후의 변수 축 attention이 이를 다시
변형하므로 최종 예측 기여도와 1:1 대응은 아니다.
